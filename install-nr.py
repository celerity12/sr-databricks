...
# Initialization script to generate installer for sr-databricks

dbutils.fs.put("dbfs:/newrelic/sr-databricks-init.sh", """ 
#!/bin/sh
...
if [ $DB_IS_DRIVER ]; then
    ...
    if ! curl -L --retry 3 --retry-delay 5 --silent --show-error -o /tmp/sr-databricks.tar.gz https://github.com/celerity12/sr-databricks/releases/download/v1.0.1/sr-databricks.tar.gz; then
      echo "Error: Failed to download sr-databricks binary."
      exit 1
    fi

    if [ -d "/etc/sr-databricks" ]; then
      rm -rf /etc/sr-databricks
    fi

    mkdir -p /etc/sr-databricks
    if ! tar xvf /tmp/sr-databricks.tar.gz -C /etc/sr-databricks; then
      echo "Error: Failed to extract sr-databricks binary."
      exit 1
    fi

    if ! python -m pip install -r /etc/sr-databricks/requirements.txt; then
    ...
    cat <<CONFIG > /etc/sr-databricks/config.yml
integration_name: com.nrlabs.databricks
run_as_service: True
poll_interval: 30
log_level: error
log_file: /tmp/sr-databricks.log
spark:
...
CONFIG
    ...
    cat /etc/sr-databricks/config.yml >> /tmp/start-databricks-metric.log

    cp --force /etc/sr-databricks/srdatabricks /etc/init.d/
    set_execute_permission /etc/sr-databricks/src/__main__.py /etc/sr-databricks/srdatabricks /etc/init.d/srdatabricks

    /etc/sr-databricks/srdatabricks start >> /tmp/srdatabricks_start.log 2>&1
